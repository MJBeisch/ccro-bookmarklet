...
function CCRORenderConvertExperimentList(experiments){var CCROvalidationCookieCheck='', ConvertVariationCookie=CCROreadCookie('_conv_v'), experimentIds=Object.keys(experiments), activeExperiments=window.convert.currentData.experiments, activeExperimentIds=Object.keys(activeExperiments), experimentLoop=0, activeStateLoop=0;
 if(experimentIds.length>0){ jQuery(".CCROoverlayuicontent").append("<table class=\"experimentlist\"><caption>Convert Experiences Experiment List</caption><thead><tr><th class=\"experiment-name\">Experiment Name</th><th class=\"variations\">Variations</th><!--<th class=\"results-link\">Results</th>--></tr></thead><tbody></tbody></table>"); for(;experimentLoop<experimentIds.length;++experimentLoop){var experimentId=experimentIds[experimentLoop], experimentName=experiments[experimentId].n, experimentVariations=experiments[experimentId].vars, experimentVariationIds=Object.keys(experimentVariations), variationLoop=0, variationOptions="";
 for(;variationLoop<experimentVariationIds.length;++variationLoop){var variationId=experimentVariationIds[variationLoop], variationName=experimentVariations[variationId].name;
if(variationName!=null){if(ConvertVariationCookie.indexOf(variationId)!=-1){variationOptions+="<option value=\""+variationId+"\" selected>"+variationName+"</option>";}
...
for(;activeStateLoop<activeExperimentIds.length;++activeStateLoop){CCROmarkActiveExperiment(activeExperimentIds[activeStateLoop]);} 
jQuery(".experimentlist .variations select").change(function(event){var selectedExperiment=jQuery(this).attr("id"),selectedVariation=jQuery(this).val();window._conv_q=window._conv_q||[];window._conv_q.push(['assignVariation',selectedExperiment,selectedVariation]);window._conv_q.push(["executeExperiment",selectedExperiment]);window.location.reload(true);});}
...
jQuery(".CCROoverlayui").remove(); jQuery("body").append('<div class="CCROoverlayui"><div class="CCROv-header"><h2>Omnitail Experiment Overlay</h2></div><div class="CCROoverlayuiscroll"><div class="CCROoverlayuicontent"></div></div><div class="CCROoverlayOptions"><div class="CCROv-ui-buttons"><button class="CCROv-toggle CCROv-button">-</button><button class="CCROv-close CCROv-button">x</button></div><label class="CCROswitch CCROsetcookie"><input type="checkbox" '+CCROvalidationCookieCheck+'><span class="CCROslider CCROround"></span></label>QC Mode</div></div>'); if(CCROsettings.toggle==0){jQuery(".CCROoverlayui").addClass('collapsed');} 
jQuery('.CCROsetcookie input').change(function(){if(jQuery(this).is(':checked')){CCROsetCookie('CCROvalidation','active',1);}
else{CCROexpireCookie('CCROvalidation');}
window.location.reload(true);}); jQuery(".CCROv-close").click(function(){CCROclose();}); jQuery(".CCROv-toggle").click(function(){CCROtoggle();}); if(window.convert){CCRORenderConvertExperimentList(window.convert.data.experiments);}}
